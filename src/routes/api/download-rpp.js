...
export const route = new AppRoute('/download-rpp', 'post', async (req, res) => {
  try {
    ...
    // let lastHeaderLevel = 0 // Commented out since it's not used in the current logic, but can be useful for future enhancements that depend on header level changes
    ...
    mdWithBoldHeading = mdWithBoldHeading
      .map((line) => {
        ...
        if (headerMatch) {
          ...
          // let lastHeaderLevel = headerLevel // Commented out since it's not used in the current logic, but can be useful for future enhancements that depend on header level changes
      ...
      (pMatch) => {
        ...
        if (cleanP.includes('<w:pPr>')) {
          cleanP = cleanP.replace(/<w:pPr\b[^>]*>[\s\S]*?<\/w:pPr>/, (pPrText) => {
            ...
            const toInject = spacingXml + indXml
            if (toInject) {
